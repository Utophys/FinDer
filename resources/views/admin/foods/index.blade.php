...
@section('content')
    <div class="mt-10 px-4">
        <div class="flex justify-between items-center mb-6 px-18">
            ...
            <button class="bg-white text-[#0E87CC] px-4 py-2 rounded hover:bg-gray-200 transition" data-bs-toggle="modal"
                data-bs-target="#createFoodModal">+ Tambah Makanan</button>
        </div>
        ...
        <div class="flex flex-wrap gap-6 justify-center max-w-7xl mx-auto">
            @foreach($foods as $food)
                ...
                <div
                    class="text-gray-800 w-64 min-h-[400px] rounded-lg shadow p-4 flex flex-col {{ $food['IS_DELETED'] == 1 ? 'bg-red-500 text-white' : 'bg-white' }}">

                    <div class="overflow-hidden whitespace-nowrap mb-2 title-container">
                        <h3 class="text-lg font-semibold inline-block" data-food-name="{{ $food['NAME'] }}">{{ $food['NAME'] }}
                        </h3>
                    </div>
                    ...
                    <div class="space-y-2 mb-4 flex-grow">
                        ...
                        <p class="text-sm"><strong>Status Deletion:</strong>
                            {{ $food['IS_DELETED'] == 1 ? 'Dihapus' : 'Aktif' }}</p>
                    </div>
                                ...
                                <div class="modal-body text-black">
                                    ...
                                    <div class="mb-4 mt-2">
                                        <label class="block text-gray-700 font-semibold mb-2"
                                            for="imageEditFood{{ $food->FOOD_ID }}">Gambar Makanan (kosongkan jika tidak ingin
                                            ganti)</label>
                                        <input type="file" id="imageEditFood{{ $food->FOOD_ID }}" name="IMAGE" accept="image/*"
                                            class="w-full p-2 border rounded">
                                        @if(!empty($food->IMAGE))
                                            <img src="{{ url($food->IMAGE) }}" class="w-full h-40 object-cover rounded mt-2"
                                                alt="food image">
                                        @endif
                                    </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body text-black">
                        ...
                        <div class="mb-4 mt-2">
                            ...
                            <input type="file" id="imageFood" name="IMAGE" accept="image/*"
                                class="w-full p-2 border rounded">
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const createForm = document.querySelector('#createFoodModal form');
            ...
            createForm.addEventListener('submit', (e) => {
                ...
                if (!nameInput.value.trim()) {
                    ...
                    errors.push("Nama makanan maksimal 255 karakter.");
                }

                // Validasi Gambar (harus ada, harus file gambar)
                if (!imageInput.files || imageInput.files.length === 0) {
                    errors.push("Gambar makanan wajib diupload.");
                } else {
                ...
                // Validasi Deskripsi (optional tapi max 1000 karakter)
